...
async function evaluate() {
    ...
    for (let positionIndex = 0; positionIndex < positions.length; positionIndex++) {
        ...
        function placeCutoff() {
            if (positionIndex <= 0) return; // Can't have a previous position if we're at index 0
        ...
        // Properly encode FEN for URL
        let queryFen = encodeURIComponent(position.fen);
        ...
        try {
            ...
            // Connection error or network issue - silently fall back to local evaluation
            placeCutoff();
            break;
        }

        if (!cloudEvaluationResponse.ok) {
            // 404 or other error - Lichess cloud eval not available for this position
            // Silently fall back to local evaluation - this is expected for many positions
            placeCutoff();
            break;
        }
        ...
        let progress =
            ((positionIndex + 1) / positions.length) * 100;
    ...
    const stockfishManager = setInterval(() => {
        ...
        if (!positions.some((pos) => !pos.topLines)) {
            ...
            // Automatically generate report after evaluation completes
            report();
