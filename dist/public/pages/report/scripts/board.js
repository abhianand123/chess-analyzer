...
function traverseMoves(moveCount) {
    ...
    let bestMoveLine = (_f = currentPosition === null || currentPosition === void 0 ? void 0 : currentPosition.topLines) === null || _f === void 0 ? void 0 : _f.find(line => line.id == 1);
    if (bestMoveLine && bestMoveLine.moveSAN) {
        $("#best-move-container").css("display", "flex");
        let bestMoveDisplay = $("#best-move-display");
        if (bestMoveLine.evaluation.type == "cp") {
            let evalText = (bestMoveLine.evaluation.value / 100).toFixed(2);
            let evalSign = bestMoveLine.evaluation.value >= 0 ? "+" : "";
            bestMoveDisplay.html(`${bestMoveLine.moveSAN} <span style="font-size: 16px; opacity: 0.8;">(${evalSign}${evalText})</span>`);
        }
        else if (bestMoveLine.evaluation.type == "mate") {
            let mateText = `M${Math.abs(bestMoveLine.evaluation.value)}`;
            bestMoveDisplay.html(`${bestMoveLine.moveSAN} <span style="font-size: 16px; opacity: 0.8;">(${mateText})</span>`);
        }
        else {
            bestMoveDisplay.html(bestMoveLine.moveSAN);
        }
    }
    else {
        $("#best-move-container").css("display", "none");
    }
    ...
    let moveSAN = (_h = (_g = positions[currentMoveIndex + (moveCount == -1 ? 1 : 0)].move) === null || _g === void 0 ? void 0 : _g.san) !== null && _h !== void 0 ? _h : "";
    if (moveSAN.endsWith("#")) {
        (_j = $("#sound-fx-check").get(0)) === null || _j === void 0 ? void 0 : _j.play();
        (_k = $("#sound-fx-game-end").get(0)) === null || _k === void 0 ? void 0 : _k.play();
    }
    else if (moveSAN.endsWith("+")) {
        (_l = $("#sound-fx-check").get(0)) === null || _l === void 0 ? void 0 : _l.play();
    }
    else if (/=[QRBN]/g.test(moveSAN)) {
        (_m = $("#sound-fx-promote").get(0)) === null || _m === void 0 ? void 0 : _m.play();
    }
    else if (moveSAN.includes("O-O")) {
        (_o = $("#sound-fx-castle").get(0)) === null || _o === void 0 ? void 0 : _o.play();
    }
    else if (moveSAN.includes("x")) {
        (_p = $("#sound-fx-capture").get(0)) === null || _p === void 0 ? void 0 : _p.play();
    }
    else {
        (_q = $("#sound-fx-move").get(0)) === null || _q === void 0 ? void 0 : _q.play();
    }
}
...
function startAutoPlay() {
    if (isAutoPlaying || !reportResults)
        return;
    isAutoPlaying = true;
    $("#play-pause-button").removeClass("fa-play").addClass("fa-pause");
    autoPlayInterval = window.setInterval(() => {
        if (!reportResults || !isAutoPlaying) {
            stopAutoPlay();
            return;
        }
        if (currentMoveIndex >= reportResults.positions.length - 1) {
            stopAutoPlay();
            return;
        }
        traverseMoves(1);
...
$("#back-start-move-button, #back-move-button, #go-end-move-button").on("click", () => {
    if (isAutoPlaying) {
        stopAutoPlay();
    }
});
